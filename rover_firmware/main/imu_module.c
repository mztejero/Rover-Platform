...
uint8_t gyro_conf_reg = 0x1B;
uint8_t acc_conf_reg = 0x1C;
...
void setup_imu_i2c(void) {
    ...
    uint8_t gyro_conf[2] = {gyro_conf_reg, 0b00001000};
    ESP_ERROR_CHECK(i2c_master_transmit(imu_dev_handle, gyro_conf, sizeof(gyro_conf), 100));

    uint8_t acc_conf[2] = {acc_conf_reg, 0b00001000};
...
void read_imu(void *arg) {
    ...
    int16_t ax_raw = ((acc_raw[0] << 8) | acc_raw[1]);
    int16_t ay_raw = ((acc_raw[2] << 8) | acc_raw[3]);
    int16_t az_raw = ((acc_raw[4] << 8) | acc_raw[5]);

    float ax = (float)ax_raw * g / LSB_PER_G;
    float ay = (float)ay_raw * g / LSB_PER_G;
    float az = (float)az_raw * g / LSB_PER_G;
    ...
    int16_t wx_raw = ((gyro_raw[0] << 8) | gyro_raw[1]);
    int16_t wy_raw = ((gyro_raw[2] << 8) | gyro_raw[3]);
    int16_t wz_raw = ((gyro_raw[4] << 8) | gyro_raw[5]);

    float wx = (float)wx_raw / LSB_PER_DEG;
    float wy = (float)wy_raw / LSB_PER_DEG;
    float wz = (float)wz_raw / LSB_PER_DEG;
